...
console.log(`🚀 Starting production QA capture for: ${BASE_URL}`);
...
async function captureProduction() {
  ...
  try {
    ...
    await page.goto(`${BASE_URL}/artists`, { waitUntil: 'networkidle' });
    await page.waitForTimeout(2000);
    await page.screenshot({ 
      path: join(SCREENSHOTS_DIR, 'artists-page.png'),
      fullPage: true 
    });
    ...
    await page.screenshot({ 
      path: join(SCREENSHOTS_DIR, 'services-page.png'),
      fullPage: true 
    });
    ...
    try {
      const mapElement = await page.locator('[data-testid="google-map"], iframe[src*="google.com/maps"], .map-container').first();
      ...
      // Check for fallback
      const fallbackElement = await page.locator('text="View on Google Maps", text="Unable to load map", [data-testid="map-fallback"]').first();
    ...
    try {
      const bookingElement = await page.locator('[data-testid="booking-button"], button:has-text("Book"), button:has-text("Appointment")').first();
...
${captureData.analytics.networkRequests.map(req => 
  `- ${new Date(req.timestamp).toISOString()}: ${req.method} ${req.url}`
).join('\n') || 'No GA4 requests detected'}

## Console Messages
${captureData.analytics.consoleMessages.map(msg => 
  `- [${msg.type}] ${msg.text}`
...
`;
  ...
  const summary = {
    ...
    summary: {
      ...
      seo5_analytics: captureData.analytics.events.length > 0 || captureData.analytics.networkRequests.length > 0
    }
  };
  
  writeFileSync(
    join(ARTIFACTS_DIR, 'capture_summary.json'),
    JSON.stringify(summary, null, 2)
  );

  console.log('\n✅ Production QA capture complete!');
  console.log('\n📊 Results Summary:');
  ...
  console.log(`└─ Scroll behavior: ${captureData.navigation.scrollBehavior.every(s => s.expectedTop) ? 'PASS' : 'FAIL'}`);
