          ...
          <AnimatePresence mode='wait'>
            {filteredImages.slice(0, 12).map((image, index) => {
              ...
              return (
                ...
                >
                  {/* Image Container */}
                  <div className='relative w-full h-full overflow-hidden bg-brand-background/50'>
                    <img
                      {...getImageProps(
                        image.src,
                        `${image.artist} - ${image.style} work`,
                        {
                          sizes: isLarge
                    ...
                    {/* Hover Overlay */}
                    <div className='absolute inset-0 bg-linear-to-t from-brand-background/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500'>
                    ...
                    <div className='absolute inset-0 border-2 border-brand-gold opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl'></div>
                  </div>
                </motion.div>
              );
            })}
