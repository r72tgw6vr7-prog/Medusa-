...
export function LuxuryButton({
  ...
  const variantClasses = {
    ...
    secondary:
      'luxury-hover bg-transparent border-2 border-brand-gold text-brand-gold hover:bg-brand-gold/10',
    ghost:
      'luxury-hover bg-transparent text-brand-chrome hover:text-brand-gold hover:bg-brand-gold/5',
  };
...
export function LuxuryCard({ children, className = '', hover = true, onClick }: LuxuryCardProps) {
  const magneticRef = useMagneticCursor<HTMLDivElement>();

  return (
    <div
      ref={onClick ? magneticRef : undefined}
      onClick={onClick}
      className={`
        card-luxury
        ${hover ? 'luxury-hover' : ''}
        ${onClick ? 'magnetic-cursor-target cursor-pointer' : ''}
        ${className}
      `}
    >
      {children}
