...
export const PageHeader: React.FC<PageHeaderProps> = ({
  ...
  return (
    <div className={`space-y-8 mb-16 ${alignClass} ${className}`}>
      {eyebrow && (
        <p className='text-sm uppercase tracking-[0.3em] text-white/50 font-semibold'>{eyebrow}</p>
      )}
      ...
      {subtitle && (
        <p
          className={`text-lg text-[#C0C0C0] font-body leading-relaxed ${maxWClass} ${marginClass}`}
        >
          {subtitle}
        </p>
      )}
