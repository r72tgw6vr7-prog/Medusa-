  ...
  return (
    ...
    >
      <div className='max-w-[1104px] mx-auto px-8 md:px-8'>
        ...
        <div className='partners-carousel-wrapper partners-fade max-w-[1100px] mx-auto px-4 md:px-6 overflow-hidden' role='region' aria-label='Partners and Press carousel'>
          <div className='partners-carousel-track flex items-center will-change-transform'>
            {loopedPartnerLogos.map((partner) => (
              <div key={partner.id} className='h-10 md:h-12 shrink-0 flex items-center justify-center max-w-[160px]'>
                <img
                  src={partner.image}
                  alt={partner.alt}
                  title={partner.name}
                  loading='lazy'
                  decoding='async'
                  className='partners-logo-img opacity-85 hover:opacity-100 transition-opacity'
                />
              </div>
            ))}
          </div>
