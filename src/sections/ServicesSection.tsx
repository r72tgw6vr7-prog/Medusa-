  ...
  return (
    <div className={`w-full bg-[#222222] ${className}`}>
      ...
      {quickLinks && quickLinks.length > 0 && (
        <div className='flex items-start justify-center py-[70px]'>
          <div className='flex gap-8 max-w-[1280px]'>
            {quickLinks.map((link, index) => (
              ...
              >
                ...
                />
                <span className='text-[#D4AF37] text-[31px] font-bold text-center mb-[15px]'>
                  {link.title}
                </span>
      ...
      {packages && packages.length > 0 && (
        <div className='flex flex-col items-center py-[70px] px-[252px]'>
          {title && <span className='text-[#D4AF37] text-[42px] font-bold mb-0.5'>{title}</span>}
          {subtitle && <span className='text-white text-[15px] mb-[55px]'>{subtitle}</span>}
