...
async function convertToWebP(imagePath) {
  ...
  try {
    ...
    console.log(`✅ Converted: ${path.relative(IMAGE_DIR, imagePath)}`);
    console.log(`   💾 Size: ${formatBytes(originalSize)} → ${formatBytes(webpSize)} (${percentSaved}% smaller)`);
  } catch (error) {
    stats.errors++;
    console.error(`❌ Error converting ${path.relative(IMAGE_DIR, imagePath)}:`, error.message);
  }
}
...
async function main() {
  ...
  console.log(`💾 Total space saved:   ${formatBytes(stats.sizeSaved)}\n`);
  ...
  if (DRY_RUN) {
    console.log('💡 Run without --dry-run to actually convert images\n');
  }
}
