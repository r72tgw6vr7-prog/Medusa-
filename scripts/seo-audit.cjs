...
console.log(`\n${colors.bold}${colors.blue}🔍 COMPREHENSIVE SEO AUDIT${colors.reset}`);
...
if (fs.existsSync(sitemapXml)) {
  ...
  test('sitemap.xml is valid XML', sitemapContent.includes('<?xml') && sitemapContent.includes('<urlset'));
  test('sitemap.xml includes homepage', sitemapContent.includes('<loc>https://www.muenchen-tattoo-studio.de/</loc>'));
  test('sitemap.xml includes services page', sitemapContent.includes('/services</loc>'));
  test('sitemap.xml includes artists page', sitemapContent.includes('/artists</loc>'));
...
if (fs.existsSync(indexHtml)) {
  ...
  test('Title includes target keyword', htmlContent.includes('Tattoo München') || htmlContent.includes('Tattoo Munich'));
  ...
  test('OG image has dimensions', htmlContent.includes('og:image:width') && htmlContent.includes('og:image:height'));
  ...
  test('Has geo coordinates', htmlContent.includes('name="geo.position"') || htmlContent.includes('name="ICBM"'));
  
  // Structured Data
  test('Has Schema.org structured data', htmlContent.includes('application/ld+json'));
  test('Has TattooShop schema', htmlContent.includes('"@type": "TattooShop"'));
  ...
  test('Has charset declaration', htmlContent.includes('charset="UTF-8"') || htmlContent.includes('charset="utf-8"'));
  test('Has noscript fallback', htmlContent.includes('<noscript>'));
  test('Noscript includes contact info', htmlContent.includes('<noscript>') && htmlContent.includes('Telefon'));
  
  // Language
  test('HTML lang attribute set', htmlContent.includes('lang="de"') || htmlContent.includes('lang="en"'));
}
...
// Check Artist cards for improved alt text
const artistCard = path.join(__dirname, '../src/components/molecules/ArtistCard.tsx');
if (fs.existsSync(artistCard)) {
  const cardContent = fs.readFileSync(artistCard, 'utf8');
  test('Artist cards have descriptive alt text', 
    cardContent.includes('spezialisiert auf') || cardContent.includes('Professional'));
  test('Artist images use lazy loading', cardContent.includes('loading="lazy"'));
}
...
if (webpPercentage < 50) {
  console.log(`   ${colors.yellow}⚠️  Only ${webpPercentage.toFixed(1)}% images are WebP. Run: node scripts/convert-to-webp.js${colors.reset}`);
}
...
if (fs.existsSync(packageJson)) {
  ...
  test('Has optimization packages', 
    pkg.devDependencies && 
...
console.log(`${colors.yellow}⚠️  Warnings: ${tests.warnings}${colors.reset}`);
...
else if (score >= 70) rating = `${colors.yellow}👍 Good${colors.reset}`;
else if (score >= 60) rating = `${colors.yellow}⚠️  Needs Improvement${colors.reset}`;
