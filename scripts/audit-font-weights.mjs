...
class FontWeightAudit {
  ...
  async run() {
    console.log('🔍 Starting Font Weight Consistency Audit...\n');
  ...
  isHardcodedWeight(weight) {
    ...
    return /^[0-9]+$/.test(weight) || 
           ['bold', 'normal', 'lighter', 'bolder'].includes(weight);
  }
  ...
  generateReport() {
    ...
    if (this.summary.totalIssues === 0) {
      console.log('✅ No font weight consistency issues found!\n');
      return;
    }
  ...
  async createFixedFiles() {
    ...
    console.log(`✅ Fix suggestions written to: ${outputPath}`);
  }
...
\`\`\`css
:root {
${Object.entries(DESIGN_TOKEN_WEIGHTS).map(([name, value]) => 
