...
console.log(`📁 Found ${imageFiles.length} image files in /public`);
console.log(`📝 Found ${imageRefs.length} image references in /src\n`);
...
const namingIssues = imageFiles.filter(file => {
  ...
  return /\s|[A-Z]|_/.test(filename) && !/^(AAron|ANGIE|Eli-luquez|Debi|Loui|Oli|Vive|Sasha)/.test(filename);
});
...
if (namingIssues.length > 0) {
  console.log('⚠️  NAMING CONVENTION VIOLATIONS:');
