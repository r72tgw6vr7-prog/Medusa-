...
function checkSpacingViolations(filePath, content, classAttr) {
  ...
  return results.map(r => ({ ...r, file: filePath, line: indexToLineNumber(content, classAttr.startIndex), code: classAttr.value.trim() }));
}
...
function checkContainerViolations(filePath, content, classAttr) {
  ...
  return out.map(r => ({ ...r, file: filePath, line: indexToLineNumber(content, classAttr.startIndex), code: classAttr.value.trim() }));
}
...
function checkCardLayoutViolations(filePath, content, classAttr, fileHasGrid) {
  ...
  }].map(r => ({ ...r, file: filePath, line: indexToLineNumber(content, classAttr.startIndex), code: classAttr.value.trim() }));
}
...
function printSummary(violations) {
  ...
  const counts = Object.entries(byType)
    .map(([type, arr]) => ({ type, count: arr.length, critical: arr.filter(v => v.severity === 'CRITICAL').length }))
...
function writeReport(violations, rootDir) {
  const report = {
    ...
    summary: {
      ...
      byType: Object.fromEntries(Object.entries(groupBy(violations, v => v.type)).map(([k, v]) => [k, v.length])),
